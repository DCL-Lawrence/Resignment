...
BEGIN_MESSAGE_MAP(CResignmentApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CResignmentApp::OnAppAbout)
	// 依據文件命令的標準檔案
...
CResignmentApp::CResignmentApp() noexcept
{
	...
	SetAppID(_T("Resignment.AppID.NoVersion"));

	// TODO:  在此加入建構程式碼，
	// 將所有重要的初始設定加入 InitInstance 中
}

// 唯一一個 CResignmentApp 物件

CResignmentApp theApp;


// CResignmentApp 初始化

BOOL CResignmentApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CResignmentDoc),
		RUNTIME_CLASS(CMainFrame),       // 主 SDI 框架視窗
		RUNTIME_CLASS(CResignmentView));
	if (!pDocTemplate)
		return FALSE;
...
int CResignmentApp::ExitInstance()
{
	// TODO: 處理其他您已經加入的資源
	AfxOleTerm(FALSE);
...
// 執行對話方塊的應用程式命令
void CResignmentApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}
